...
public class Bootstrap {
    ...
    private static final String ARG_CONFIG_DIR = "-configDir";
    ...
    private static void readConfigDirArgument(String[] args) {
        for (int i = 0; i < args.length; i++) {
            if (ARG_CONFIG_DIR.equals(args[i]) && i + 1 < args.length) {
                System.setProperty(ShellImpl.PROP_FORGE_CONFIG_DIR, args[i + 1]);
                return;
            }
        }
    }
    ...
    public void observeStart(@Observes final Startup event, final Shell shell) throws Exception {
        if(!isIntrabundleInstalled()){
            shell.println(ShellColor.YELLOW,"Auto installing intrabundle plugin from github repo, make sure you have internet access\n use command 'forge intrabundle-update' to re-install the plugin from github ");
            shell.execute("forge git-plugin git://github.com/rmpestano/intrabundle.git");
       }
    }

    private boolean isIntrabundleInstalled() {
